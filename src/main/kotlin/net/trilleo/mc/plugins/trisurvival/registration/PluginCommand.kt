 ...
 * `net.trilleo.mc.plugins.trisurvival.commands` package (or any subpackage) to
 * have it automatically discovered and registered at startup.
 *
 * By default every command is registered as a **sub-command** of `/trisurvival`
 * (alias `/ts`). For example, a command with `name = "reload"` becomes
 * `/trisurvival reload`. Set [isMainCommand] to `true` to register the command
 ...
 *
 * Example (sub-command – registered as `/trisurvival start`):
 * ```kotlin
 ...
 *     usage = "/trisurvival start",
 *     permission = "trisurvival.start"
 * ) {
