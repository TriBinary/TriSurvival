...
object PureCoal : CustomOre("pure_coal") {

    override val representingBlock: Material = Material.COAL_BLOCK
    override val blockStrength: Int = 200
    override val dropVanillaItem: Material = Material.COAL
    override val baseDropAmount: Int = 3
    override val skillXp: Double = 10.0
    override val expOrbDrop: Int = 3
    override val requiresPickaxe: Boolean = true
    override val minToolTier: ToolTier = ToolTier.STONE
    ...
    override fun generate(chunk: Chunk, world: World, random: Random) {
        val attempts = random.nextInt(3, 6)
        repeat(attempts) {
            val baseX = random.nextInt(16)
            val baseZ = random.nextInt(16)
            val baseY = random.nextInt(-64, 64)
            val veinSize = random.nextInt(4, 6)
            repeat(veinSize) {
