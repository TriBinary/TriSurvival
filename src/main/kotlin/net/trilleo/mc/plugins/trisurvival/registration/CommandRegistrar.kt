 ...
 *   the `/trisurvival` parent command (alias `/ts`). A command with
 *   `name = "reload"` becomes `/trisurvival reload`.
 * * **Main commands** (`isMainCommand = true`) – registered directly on the
 *   server [CommandMap] as standalone top-level commands.
 *
 * The `/trisurvival` command provides tab-completion for all registered
 * sub-commands automatically.
 */
object CommandRegistrar {

    private const val COMMANDS_PACKAGE = "net.trilleo.mc.plugins.trisurvival.commands"
    private const val ROOT_COMMAND = "trisurvival"
     ...
     * @property isSubCommand `true` when the command is a sub-command of `/trisurvival`
     */
     ...
     * and registers it either as a sub-command of `/trisurvival` or as a
